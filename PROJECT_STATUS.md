...
- **Per-level procedural soundtrack**: an ambient drone/theme (distinct interval, tremolo, dissonant layer per level) plus, layered on top during actual gameplay, a **Melbourne-bounce-style rhythm track** with its own bpm/step-pattern/root-pitch per level (`BOUNCE_PARAMS`) — all ten rounds sound different, not just "faster."
...
- Last commit pushed: `afe029f` — "Rename cable throw to burning candle, add per-level bounce beats and flame row".
